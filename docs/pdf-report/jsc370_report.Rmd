---
title: "JSC370 Final: A Look at NFL Field Goals in 2018-2020"
author: "Chloe Nguyen"
...
The repo link for this project is: https://github.com/annguyen1404/JSC370-final

The website link for this project is: https://annguyen1404.github.io/JSC370-final/
...
The dataset we're working with details all special team plays (that is, plays that involve punts, kickoffs, field goals and extra points) in all National Football League (NFL) games from 2018 to 2020. This dataset was provided by the NFL as a part of their 2022 data competition, Big Data Bowl: [click for link](https://www.kaggle.com/c/nfl-big-data-bowl-2022/overview). We also refer to this dataset for the locations of NFL teams' home stadiums: [click for link](https://github.com/Sinbad311/CloudProject/blob/master/NFL%20Stadium%20Latitude%20and%20Longtitude.csv). Additionally, we use this dataset for the abbreviations of NFl teams:
[click to link](https://gist.github.com/cnizzardini/13d0a072adb35a0d5817).
...
tibble(`Number of Rows` = c(nrow(teams_info)),
  ...
  kbl(caption = "Dimensions of Merged Dataset of NFL Team Abbreviations and Locations", align='c',  booktabs = T) %>%
  kable_styling(bootstrap_options = c("striped"), full_width = F, font_size = 9, latex_options = "hold_position")
```
...
tibble(`Datasets`= c("Games", "Merged"),
  ...
  kbl(caption = "Number of Observations in Datasets", align='c',  booktabs = T) %>%
  kable_styling(bootstrap_options = c("striped"), full_width = F, font_size = 9, latex_options = "hold_position")
...
Now, we select the variables relevant to our question of interest: season, week, gameDate, gameTimeEastern, longitude, latitude, homeTeamAbbr, visitorTeamAbbr, quarter, down, specialTeamsPlayType, specialTeamsResult, possessionTeam, yardlineNumber, gameClock, kickLength, kickerId, height_cm, weight_kg, birthDate, displayName, collegeName, Position. With that, we create the following variables of interest: the birth year of player kicking the ball (birthYear), the age of player at the time of kick (age), whether or not the kick attempt was classified as "good" (specialTeamsSuccess), whether or not the team in posession of the ball is the home team (possessionTeamHome). Finally, we impute missing values for relevant variables. Note that we use the mean for numerical values and the median for categorical values. The dataset is now ready to be explored.
...
tibble(`Column` = colnames(data),
  ...
  kbl(caption = "Overview of Variables") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F, font_size = 9, latex_options = "hold_position")
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', out.height='200px'}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', out.height='150px'}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', out.height='160px'}
...
Next, we visualize the teams and their home stadiums on a map. This map is available on the website.
...
# Table for summary
kbl(test, 
    caption = "Mann–Whitney U test - High vs. Low Elevation", 
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.height='150px'}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.height='200px'}
colour <- colorRampPalette(c("#355C7D", "#C06C84"))

bag_plot <- ggplot(imp, aes(x=reorder(varnames, IncNodePurity), weight=IncNodePurity)) + 
...
rf_plot <- ggplot(imp, aes(x=reorder(varnames, IncNodePurity), weight=IncNodePurity)) + 
  geom_bar(fill=colour(nrow(imp)), alpha = 0.7) +
  ylab("IncNodePurity") +
  xlab("Variable Name") +
  coord_flip() +
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, out.height='200px', out.width='350px'}
grid.arrange(bag_plot, rf_plot, ncol=2)
```
